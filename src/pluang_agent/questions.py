...
R7 added `synthesize_business_question()` so the `pluang-agent ask` CLI can
...
def synthesize_business_question(
    ...
    *,
    ...
    Used by the `pluang-agent ask` CLI surface. When metric or period are
    ...
    from pluang_agent.planner import infer_metric_intent, infer_period_from_text
